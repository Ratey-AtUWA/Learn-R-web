...
```{r ggplot-scatter-reglines, fig.height=3.5, fig.width=5, message=FALSE, warning=FALSE, fig.cap="Figure 1: X-Y (scatter) plot of calcium (Ca) vs. pH for different sample types from Smith's Lake and Charles Veryards Reserves in 2017 (`sv2017`), showing regression lines by Type."}
...
* specify the data to be plotted in the `ggplot()` function, including a data frame with the `data=` option, and which variables control the location and appearance of points with the options in `mapping=aes(...)` (`aes` is short for “aesthetic”)
...
* add a layer of regression “best fit” lines using `geom_smooth(method="lm")` with options
...
`geom_smooth(method="lm")` in `ggplot` to add a regression line:

```{r gg-regline, fig.width=4, fig.height=4, fig.cap="Plot of Zn vs. Fe, with no grouping and an added regression line.", message=FALSE, warning=FALSE, results='hold'}
ggplot(data=sv18, aes(x= Fe, y = Zn)) +
  geom_point() +
...
The grey shaded area around the regression line in Figure \@ref(fig:gg-regline)
is the 95% confidence interval, which is included by default.

We can also include trend lines for grouped scatterplots (Figure 
\@ref(fig:gg-reglines-grouped)):

```{r gg-reglines-grouped, fig.width=5.5, fig.height=4, fig.cap="Plot of Zn vs. Fe, with added regression lines for data grouped by a sample Type.", message=FALSE, warning=FALSE, results='hold'}
ggplot(data=sv18, aes(x=Fe, y=Zn, colour=Type, shape=Type)) +
  geom_point(size=2.5) +
  geom_smooth(method="lm") +
  labs(x="Fe (mg/kg)", y="Zn (mg/kg)") +
  scale_x_log10() + scale_y_log10() +
...
```{r gg-depth-profile, fig.cap="Figure 17: ‘Depth profile’ plots of Fe concentration vs. depth measurements in the sv18 data.", fig.height=5, fig.width=3.6, message=FALSE, warning=FALSE}
