...
```{r plot-nmds-all-Jaccard-2D, fig.height=6, fig.width=6, fig.cap="Figure 1: 2D NMDS ordination plot based on Jaccard (dis)similarities in eDNA species presence-absence data for water sampled from Ashfield Flats in 2022.", out.width="70%"}
...
legend(s3d$xyz.convert(-1.6, -1.3, 1.4), legend = levels(eDNA2022$Site), bty="n",
      title="Site", pt.bg= c(1:11), pt.cex=1.4, col="gray", pch=c(rep(21:25, 2),21))
```
...
```{r basic-pco-plot, fig.height=5, fig.width=5, fig.cap="Figure 3: Basic principal coordinates analysis (PCoA) ordination plot based on Jaccard (dis)similarities in eDNA species presence-absence data for water sampled from Ashfield Flats in 2022."}
par(mfrow=c(1,1), mar=c(3,3,2,0.5), mgp=c(1.5,0.2,0), tcl=0.2, font.lab=2)
plot(AF_pco_all)
```
...
```{r better-pco-plot-s3d, fig.width=4, fig.height=4, out.width="70%", fig.cap="Figure 4: Principal coordinates (PCoA) ordination plot in 3D based on Jaccard (dis)similarities in eDNA species presence-absence data for water sampled from Ashfield Flats in 2022.", echo=1:3}
palette(c("black", plasma(10))) # a viridis palette
par(mfrow = c(1,1), mar = c(3,3,1,1), mgp = c(1.5,0.2,0), tcl = 0.25, 
    font.lab = 2)
scatterplot3d(x=AF_pco_all$points[,1], y=AF_pco_all$points[,2], z=AF_pco_all$points[,3],
              xlab="Dim 1", ylab="Dim 2", zlab="Dim 3",
              angle=65, mar=c(4,4,1,2), type="h", col.axis="gray", color="#e0e0e0",
              pch=c(rep(21:25, 2),21)[eDNA2022$Site],
              bg=c(1:11)[eDNA2022$Site], cex.symbols=1.4,
...
assumption of normality &ndash; generally speaking, biological species data are not
...
run &ndash; luckily now it only takes a matter of seconds to minutes depending on the
...
```{r stress-plot, fig.height=5, fig.width=7, out.width="50%", fig.cap="Figure 5: Stress plot showing the goodness-of-fit between actual dsiimilarity for the Ashfield Flats 2022 eDNA data and dissimilarities predicted by NMDS ordination distances.", message=FALSE, warning=FALSE}
...
AF_nmds_aquat
```
```{r plot nmds aquat Bray, fig.height=4, fig.width=4, fig.cap="Figure 6: Basic NMDS plot showing observations as symbols and species as text. Presence-absence data from eDNA sampling and analysis at Ashfield Flats."}
par(mfrow=c(1,1), mar=c(3,3,2,0.5), mgp=c(1.5,0.2,0), tcl=0.2, font.lab=2)
...
```{r alt-to-vegan-nmds-aquat, fig.height=3.5, fig.width=7, fig.cap="Figure 7: Two-dimensional non-metric multidimensional scaling (NMDS) plot showing observations as symbols and species as text located in MDS2 vs. MDS1 space. Presence-absence data for aquatic species only from eDNA sampling and analysis at Ashfield Flats."}
palette(c("black", plasma(10))) # a viridis palette
par(mfrow=c(1,2), mar=c(3,3,0.5,0.5), mgp=c(1.5,0.2,0), tcl=0.2, font.lab=2)
...
legend("left", legend=seq(1,11), bty="n", title="Site", cex=1, pt.cex=1.2, 
...
```{r decorana-plot, fig.height=4, fig.width=4, fig.cap="Figure 8: Two-dimensional detrended correspondence analysis (DCA) plot showing observations as symbols and species as text. Presence-absence data from eDNA sampling and analysis at Ashfield Flats."}
par(mfrow=c(1,1), mar=c(3,3,0.5,0.5), mgp=c(1.5,0.2,0), tcl=0.2, font.lab=2)
plot(AF_dca_all)
```
