...
- For horizontal transport use Decline from Vertical&hellip;; of ≤ 0.1
...
`Frac` = &hellip;;fraction of adsorption sites with instantaneous sorption &hellip;; Set = 1 for equilibrium transport  
`ThIm` = Immobile water content. Set equal to 0 when *physical nonequilibrium* is not considered.

#### `Solute Specific Parameters`:
`Diffus.W` = diffusion coefficient in water, *D~w~* [L^2^T^-1^]  
`Diffus.G` = diffusion coefficient in soil air, *D~a~* [L^2^T^-1^]
...
- Click `Pressure Head&hellip;;`, then click the `Edit Condition` button, and select the whole column by clicking top then bottom, then set `Top` and `Bottom` value to 0 (zero) *i.e*. **saturated with water**
