...
When the `Profile Information` window is saved and closed, the 
...
colnames(hydrus0)[seq(3,ncol(hydrus0),2)] <- paste0("T",round(simTimes))
```
...
### Plot the data (option 1: symbols and lines)
...
```{r plot-melted-2, fig.width=8, fig.height=4, fig.cap="Figure 3: Plot of simulated solute concentration vs. horizontal distance for each simulation time step (showing smooth lines only), using Hydrus-1D", results='hold'}
par(mar=c(4,4,1,1), mgp=c(1.7,0.3,0), tcl=0.25, 
    font.lab=2, cex.lab=1.2)
...
```{r hydrus-plot-gg, fig.width=10, fig.height=4, fig.cap="Figure 4: Plot of simulated solute concentration vs. horizontal distance for each simulation time step (the `ggplot` version), using Hydrus-1D", results='hold'}
require(ggplot2)
ggplot(hydrus1) +
  geom_line(aes(x=Distance, y=Concentration, colour=Time,
                linetype=Time), linewidth=1.2) +
  scale_color_manual(values=c("grey",viridis::turbo(9, beg=0.05))) +
  guides(color=guide_legend(title="Simulation time")) +
  guides(linetype=guide_legend(title="Simulation time")) +
...
```{r conc-hdist, fig.width=6, fig.height=4, fig.cap="Figure 6: Plot of simulated solute concentration vs. horizontal distance for each simulation time step, using Hydrus-1D", results='hold'}
