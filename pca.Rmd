...
Alternatively we could look at a 3D plot using the `scatterplot3d` package.

```{r fig.height=5, fig.width=12, fig.cap="Plot of (a) observation scores and (b) component (variable) loadings for PCA on clr-transformed sediment elemental concentrations from Ashfield Flats 2019-2021."}
...
text(s3d$xyz.convert(pca_ashfield_open$rotation[, 1:3]), labels = rownames(pca_ashfield_open$rotation),
     cex= 0.85)
text(s3d$xyz.convert(0,0,0), labels = "(b)")
```
