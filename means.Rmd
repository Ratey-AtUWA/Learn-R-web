...
author: "Andrew Rate, School of Agriculture and Environment, UWA"
date: "`r Sys.Date()`"
output: 
  html_document:
    highlight: kate
...
```{r SLCVR-map, echo=FALSE, fig.width=5, fig.height=4.7, fig.cap="Map showing locations of Charles Veryard and Smiths Lake Reserves, North Perth, Western Australia (location sampled for the sv2017 dataset).", fig.align='center', results='hold',warning=FALSE}
...
shadowtext(c(391365,391260), c(6466370,6466660),
     ...
     col="darkgreen", bg="white", font=3)
lines(st_bbox(SLtiles)[c(1,3)],rep(6466530,2), lty=1, lwd=8, col="#FFFFFFC0")
lines(st_bbox(SLtiles)[c(1,3)],rep(6466530,2), lty="23", lwd=4, col="purple3")
shadowtext(391480,6466532,label="Northing\n6466530", 
           col="purple3", bg="white")
par(xpd=FALSE)
```
...
We can also draw the plot using the `ggplot2` package:

```{r xyMap-GG, fig.height=4, fig.width=4.4, out.width='40%', fig.align='center', results='hold', fig.cap="Location map of soil samples at Charles Veryard and Smiths Lake Reserves made with `ggplot`."}
library(ggplot2)
...
boxplot or density plot (Figure \@ref(fig:vis-variance-2group); we need the 
`car` package for the `densityPlot()` function):
...
densityPlot(Na.pow ~ Reserve, data=sv17_soil, 
...
TIn each case it's apparent that the variance in Na in the Veryard soil is 
similar to that at Smith's Lake, illustrating the conclusion from the 
statistical tests.
</div>

he power term from `powerTransform()` is &sime;&nbsp;0.127, so we make a new
...
We can visualize means comparisons in a few different ways &ndash; see Figure 
...
```{r meancomp-plots-2-groups, fig.height=3, fig.width=6, message=FALSE, warning=FALSE, fig.cap="Graphical comparison of means between groups using: (a) a standard box plot; (b) a box plot also showing mean values in each group. You would not include both options in a report!"}
...
The `ggplot` connoisseurs can also draw such a boxplot:

```{r 2way-box-gg, fig.height=3, fig.width=4, fig.align='center', fig.cap="Boxplot for 2-group comparison, showing means, and drawn with `ggplot2`.", message=FALSE, warning=FALSE}
...
flextable(cohens) |> theme_zebra(odd_body = "lightcyan") |> delete_part(part="header") |> 
  width(width=c(1.2,0.5,1.5,2), unit="cm") |>
  align(j=1, align="right", part="all") |> align(j=2, align="c") |> align(j=3, align="left") |> 
  border_outer(border=fp_border(),part="all") |> border(i=1,border.top = fp_border())
```
...
```{r test variance for groups in Type, echo=2, results='hold'}
...
```{r vis-var-3groups, fig.height=3.5, fig.width=7, fig.align='center', fig.cap="Graphical visualization of variance (the spread of the distribution) in 3 groups using (left) a box plot and (right) a density plot.", out.width="65%", echo=-5}
require(car)
par(mfrow=c(1,2), mar=c(3,3,0.5,0.5), mgp=c(1.6,0.5,0),
    font.lab=2, font.main=3, cex.main=0.8, tcl=-0.2,
    cex.lab = 1, cex.axis = 1)
boxplot(sv2017$Ca.pow ~ sv2017$Type, 
        notch=FALSE, col="gainsboro",
        xlab="Reserve", ylab="Ca (power-transformed")
densityPlot(sv2017$Ca.pow ~ sv2017$Type, adjust=2, 
            xlab="Ca (power transformed)", ylim=c(0,18))
par(mfrow=c(1,1)) # reset multiple graphics panes
```

In each case it seems that the variance in power-transformed Ca is Sediment >
Soil > Street dust (Figure \@ref(fig:vis-var-3groups)). We can check the actual
variance values using `tapply()`, and this confirms our interpretation:
...
```{r meancomp-plots-3groups, fig.height=3, fig.width=6, fig.cap="Graphical comparison of means between 3 or more groups, in three ways: (a) a notched box plot (notches are approximate 95% confidence intervals around the median); (b) a box plot also showing mean values in each group.", message=FALSE, warning=FALSE}
par(mfrow=c(1,2), mar=c(3.5,3.5,1.5,1.5), mgp=c(1.7,0.3,0),
    font.lab=2, font.main=3, cex.main=1, tcl=0.2,
    cex.axis=0.9, lend = "square", ljoin = "mitre")
boxplot(sv2017$Ca.pow ~ sv2017$Type, notch=T, 
...
### Visualsing 3-group differences with `ggplot2`

```{r 3way-box-gg, fig.height=3, fig.width=4.5, fig.align='center', fig.cap="Boxplot for 3-group comparison, showing means, and drawn with `ggplot2`.", message=FALSE, warning=FALSE}
