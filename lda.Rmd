...
machine learning). Towards the end of this section we will see if we can divide
our dataset into two groups – one to 'train' our LDA model to generate a set of
linear discriminant functions, which we can then apply to the remaining
observations in our dataset to *validate* our LDA model.
...
Hallberg_clr[,11:24] <- t(apply(Hallberg[,11:24], MARGIN = 1,
                           FUN = function(x){log(x) - mean(log(x))}))
print(round(head(Hallberg_clr[,11:24]),3))
```
...
element oxide contents, SiO~2~, TiO~2~, Al~2~O~3~, Fe~2~O~3~, FeO, MnO, MgO, 
CaO, Na~2~O, K~2~O, & P~2~O~5~. We're excluding LOI, CO~2~ and H~2~O, since
these variables contain too many similar low values which adversely affects the
matrix algebra calculation of the linear discriminant analysis procedure.
...
cmx[lower.tri(cmx)] <- NA ; print(cmx, na.print=".")
rm(cormat)
```
...
LD1 explains 0.845 (85%) and LD2 explains 0.093 (9.3%) of variance between Rock 
type categories).
...
side-by side comparison) are shown in Figure 1.


```{r ldahistComp, fig.align='center', fig.cap="Figure 1: Histograms based on the first linear discriminant function for (a) closed and (b) open (CLR-transformed) whole-rock major element data.", fig.height=14, fig.width=10, out.width="80%", results='hold'}
...
The sets of histograms for closed and open data in Figure 1 both
...
```{r pplotComp, fig.height=5, fig.width=10, fig.align='center', out.width="100%", fig.cap="Figure 2: Partition plots for (a) closed and (b) open (CLR-transformed) whole rock major element data, based on the SiO₂ and TiO₂ contents. Filled symbols are means in each category, with red letters showing apparently mis-classified observations.", results='hold'}
require(klaR)
...
Partition plots, such as those  in Figure 2, are presented for single
pairwise combinations of the variables (in this example SiO~2~ and
TiO~2~) used to make the LDA model. We can make differnt plots by
...
```{r Fig3-LDAclos, fig.height=8, fig.width=8, fig.align='center', out.width="80%", fig.cap="Figure 3: Linear discriminant analysis (LDA) plots for closed rock composition data: (a) variable coefficients in LD1-LD2 space, and predictions for observations in (b) LD1-LD2 space; (c) LD1-LD3 space; (d) LS2-LD4 space. Legend in (a) applies to plots in (b), (c), and (d).", results='hold'}
...
```{r Fig4-LDAopen, fig.height=8, fig.width=8, out.width="70%", fig.align='center', fig.cap="Figure 4: Linear discriminant analysis (LDA) plots for open (CLR-transformed) rock composition data: (a) variable coefficients in LD1-LD2 space, and predictions for observations in (b) LD1-LD2 space; (c) LD1-LD3 space; (d) LS2-LD3 space. Legend in (a) applies to plots (b), (c), and (d).", results='hold'}
par(mfrow = c(2,2), mar = c(3.5,3.5,1,1), mgp = c(1.5,0.3,0), tcl = 0.25,
    lend = "square", ljoin = "mitre", cex.main = 0.9, font.lab=2)
...
From the plots in Figure 3 and Figure 4, we can see that the LDA models obtained
are certainly able to separate observations by the selected factor. Firstly,
however, there is a lot of clustering of the predictor variables in Figure 3(a),
which may relate to spurious relationships between variables caused by
compositional closure. This clustering is not so pronounced in Figure 4(a),
...
```
 ...
 code below uses the `head()` function to inspect the first few rows of each
...
```{r loocv-LDA, message=FALSE, warning=FALSE, paged.print=FALSE}
...
flextable(matches[1:10,]) |> bold(part="header") |> width(width=c(4.8,2.8,3.8), unit="cm") |> 
  ...
  set_caption(caption="Table 1: Matches between LDA model and actual data based on clr-transformed major element concentrations in the whole rock dataset compiled by Hallberg (2006). Only the first 10 observations are shown.")
```
...
Based on Figure 5, it looks like we should run at least 50-100 iterations to get
...
```{r accuracy-vs-iterations, fig.height=3.5, fig.width=5, fig.align='center', message=FALSE, warning=FALSE, out.width="40%", fig.cap="Figure 5: Accuracy (shown by 95 percent CI error bars) as a function of number of train-validate iterations for an LDA model using closed whole-rock composition data.", results='hold'}
...
for (i in 1:n0) {
  ...
  train <- sample(1:NROW(Hallberg_clr), round(NROW(Hallberg_clr) * ftrain,0))
      if (is.na(match(NA,tapply(Hallberg_clr[train,]$SiO2, 
